...
count = 0
friends_timeline_result = client.statuses.friends_timeline.get(count=100)
print friends_timeline_result.keys()
statuses = friends_timeline_result['statuses']
for status in statuses:
    count += 1
    for word in category_keywords['business']:
        if 'retweeted_status' in status:
            status = status['retweeted_status']

        if status['text'].find('http') < 0:
            continue
        #print status['text']
        text = remove_hashtag(status['text'])
        #print text
        match = get_http(text)
        if match:
            link = text[match.start():match.end()]
            text = text.replace(link, '')
            #print text
            print remove_column(text)
            #print link
            
        """
        if status['text'].find(word) >= 0:
            print status['text']
        """
